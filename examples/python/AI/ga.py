...
def test():
    ...
    if yesno("Do you want to evolve a list of integers to maximize their sum?"):
        ...
        if yesno("Continue evolving?"):
            ga.evolve()
            print("Testing init from file (load all with mutate)...",)
            print("reInitialize pop...")
            ga.reInitialize()
            ga.initGenesFromFile("maxsumga.genes")
            ga.evolve()
            print("Testing init from file (load 1 no mutate)...",)
            ga.saveGenesToFile("bestsumga.genes", (ga.pop.bestMember.position,))
            ga.reInitialize()
            ga.initGenesFromFile("bestsumga.genes", 1, 0)
            ga.evolve()
            print("Testing init from file (load 1, with mutate, full)...",)
            #sys.stdin.readline()
            ga.reInitialize()
