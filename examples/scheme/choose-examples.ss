...
(define sum
   (lambda args
      (cond
        ((= (length args) 0) 0)
        (else (+ (car args) (apply sum (cdr args)))))))

(define magic-square
   (lambda ()
        (let ((c1 (choose 1 2 3 4 5 6 7 8 9))
              (c2 (choose 1 2 3 4 5 6 7 8 9)))
          (require (distinct? (list c1 c2)))
          (let* ((c3 (choose 1 2 3 4 5 6 7 8 9))
                 (sum1 (sum c1 c2 c3)))
             (require (distinct? (list c1 c2 c3)))
             (let ((c4 (choose 1 2 3 4 5 6 7 8 9)))
                (require (distinct? (list c1 c2 c3 c4)))
                (let ((c5 (choose 1 2 3 4 5 6 7 8 9)))
                  (require (distinct? (list c1 c2 c3 c4 c5)))
                  (let* ((c6 (choose 1 2 3 4 5 6 7 8 9))
                         (sum2 (sum c4 c5 c6)))
                    (require (distinct? (list c1 c2 c3 c4 c5 c6)))
                    (require (= sum1 sum2))
                    (let* ((c7 (choose 1 2 3 4 5 6 7 8 9))
                           (sum4 (sum c1 c4 c7))
                           (sum7 (sum c3 c5 c7)))
                      (require (distinct? (list c1 c2 c3 c4 c5 c6 c7)))
                      (require (= sum2 sum4 sum7))
                      (let* ((c8 (choose 1 2 3 4 5 6 7 8 9))
                             (sum5 (sum c2 c5 c8)))
