...
globals()['MAXREQUESTLENGTH'] = 100000 # maximum allowed length of XML-RPC request (in bytes) 100000 
...
if globals()['cmd_options'].password == None:
    globals()['RPC_PASSWORD'] = sha.new(str(time.time())+globals()['SOCKER_TGUID']+sha.new(str(time.time())).hexdigest()).hexdigest()
else:
    globals()['RPC_PASSWORD'] = globals()['cmd_options'].password


if globals()['cmd_options'].enable_debug == True: print "[SECURITY] RPC_PASSWORD SET TO [%(pass)s]"%{'pass':globals()['RPC_PASSWORD']}


#Temp lang stuff

DEFAULT_LANG = cmd_options.language # Changed to the --lang flag
...
class Session:
    ...
    def addName(self, name, password):
        for a_registered_server in self._owner.servernames:
            if "_db" not in dir(self._owner.DB):
                self.DEBUG("DB: has no database", 'error')
                return 0
            ### DSB: My Database Interface
            self._owner.DB.addUser(a_registered_server, name, password)
        return 1
...
#                    self.DEBUG('server','Plugging-out?','info')
        ...
        if globals()['cmd_options'].socker_info != None:
            self._socker = Socker_client(self,globals()['cmd_options'].socker_info,globals()['SOCKER_TGUID'])
