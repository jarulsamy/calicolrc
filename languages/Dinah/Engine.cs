...
namespace Dinah
{
	...
	public class CallStack
	{
		...
		public void AppendRunner(Statement b) {
	...
	public class Engine
	{
		...
		public bool Reset() //Jigsaw.Canvas cvs, InspectorWindow inspector)
		{
			...
			List<Statement> startStatements = Program.getStartStatements();
			
			foreach (Statement s in Program.getAllStatements()) {
				s.blockState = BlockState.Idle;	// Move these statements to a CBlock method called Reset()
				if (startStatements.Contains(s)) {
					CallStack callstack = new CallStack(globals, builtins);
					callstack.AppendRunner( s );
					_callStacks.Add(callstack);
					List<IEnumerator<RunnerResponse>> stack = new List<IEnumerator<RunnerResponse>>();
					locals = new Scope();
					stack.Add( s.Runner(locals, builtins) );
				}
			}
