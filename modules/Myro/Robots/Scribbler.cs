...
public class Scribbler: Myro.Robot
{
    ...
    static byte ENABLE_PAN     = 13;  // Fluke2 only
    ...
    public override void setPictureSize(string size)
    {
        ...
        {
            Console.WriteLine("Unrecognized parameter. Valid parameters are 'small' and 'large'.");
            return;
        }
    ...
    public byte get_cam_param (int addr)
    {
        write(Scribbler.GET_CAM_PARAM);
        write((byte)addr);
        return read_byte();
    }

    public override void darkenCamera(int level)
    {        
        if (isFluke2())
        {
            level += 128;
            set_cam_param(0, level);
        }
        else
        {
    ...
    public override  void autoCamera()
    {
        if (isFluke2())
        {
            set_cam_param(0x0D, 0x01);
        }
        else
        {
            set_cam_param(0, 0);
            set_cam_param(1, 0x80);
            set_cam_param(2, 0x80);
            set_cam_param(6, 0x80);
            set_cam_param(0x10, 0x41);
            set_cam_param(CAM_COMA, CAM_COMA_DEFAULT);
            set_cam_param(CAM_COMB, CAM_COMB_DEFAULT);            
        }
    }
